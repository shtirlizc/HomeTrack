...
const defaultCreateState: RegionCreateInput = {
  title: "",
  description: "",
};
...
export const RegionsTable: FC<Props> = ({ regions }) => {
  ...
  const [creatingValues, setCreatingValues] =
    React.useState<RegionCreateInput>(defaultCreateState);
  const handleCreate = async (data: RegionCreateInput) => {
    startTransition(async () => {
      createFormAction(data);
    });
  };

  const [editingValues, setEditingValues] = React.useState<Region | null>(null);
  const handleEdit = (region: Region) => {
    setEditingValues(region);
  };
  const handleCancelEdit = () => {
    setEditingValues(null);
  };
  const handleSaveEdit = async (region: RegionCreateInput) => {
    startTransition(async () => {
      updateFormAction(region);
    });
  };

  const handleDelete = async (regionId: string) => {
    startTransition(async () => {
      deleteFormAction(regionId);
    });
  };
